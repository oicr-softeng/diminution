...
export const csvToJSON = (csv = '', {
    creation = new Date(),
    destinationColumnName = 'longUrl',
    replacements = {},
}) => {
    const rows = csv
        .split(/\r?\n/g)
        .filter(row => row); // removes empty lines

    const headers = rows[0]
        .split(',')
        .map(header => (header[0].toLowerCase() + header.slice(1)).trim());

    const destinationIndex = headers.findIndex(header => header.toLowerCase() === destinationColumnName.toLowerCase());

    return rows
        .slice(1) // removing the headers first (not an actual row)
        .map(row => { // breaking them into columns
            ...
            return firstAttempt.length > headers.length // we should handle a row getting jumbled by unencoded urls
                ? firstAttempt.slice(0, destinationIndex) // so, we get columns before the destination
                    .concat( // appending the destination itself
                        encodeURI( // encoding it this time
                            cleanURL( // and ensuring the URL starts with http(s)
                                firstAttempt.slice(
        ...
        .filter((row, rowIndex) => row.length === headers.length || ( // if any row was invalid for any reason
            process.env.DEBUG && console.log('discarded', rowIndex + 2, row) // row number based on the original CSV
        ))
        .map(row => headers.reduce((acc, header, column) => {
            ...
            return value
                ...
                : (
                    process.env.DEBUG && console.log('possibly corrupted value for', header, value, '\nin row\n', row),
                    acc
                );
        }, { creation }));
};
...
export const uploadedURLsShaper = data => data.map(({
    creation,
    longUrl,
    lastUsed,
    timesUsed,
